const std = @import("std");
const bi = @import("bigint.zig");

const Fe = bi.Fe;
const N = bi.N;

// An RSA-4096 CRT signing key whose limbs and Montgomery constants are produced
// at runtime. Everything here is computed once, before the benchmark's timing
// loop, so the per-signature cost reflects only the modular exponentiation with
// a modulus the compiler never saw.
pub const Key = struct {
    p: Fe,
    q: Fe,
    p_exp: Fe, // dp = d mod (p-1)
    q_exp: Fe, // dq = d mod (q-1)
    p_n0inv: u64,
    q_n0inv: u64,
    p_rr: Fe, // R^2 mod p
    q_rr: Fe, // R^2 mod q
    qinv_mont: Fe, // (q^-1 mod p) * R mod p

    pub fn fromHex(
        p_hex: []const u8,
        q_hex: []const u8,
        dp_hex: []const u8,
        dq_hex: []const u8,
        qinv_hex: []const u8,
    ) !Key {
        const p = try hexToFe(p_hex);
        const q = try hexToFe(q_hex);
        const p_rr = computeRR(&p);
        const q_rr = computeRR(&q);
        const qinv = try hexToFe(qinv_hex);
        return .{
            .p = p,
            .q = q,
            .p_exp = try hexToFe(dp_hex),
            .q_exp = try hexToFe(dq_hex),
            .p_n0inv = negInv64(p[0]),
            .q_n0inv = negInv64(q[0]),
            .p_rr = p_rr,
            .q_rr = q_rr,
            // qinv * R mod p = montMul(qinv, R^2) since montMul folds in one R^-1.
            .qinv_mont = bi.montMul(&qinv, &p_rr, &p, negInv64(p[0])),
        };
    }
};

// Big-endian hex (up to 2048 bits) into little-endian 64-bit limbs.
fn hexToFe(hex: []const u8) !Fe {
    if (hex.len > 2 * 8 * N) return error.KeyTooLong;
    var bytes: [8 * N]u8 = @splat(0);
    // Right-align the value so short (leading-zero-trimmed) hex still lands in
    // the low end of the big-endian buffer.
    const nbytes = hex.len / 2;
    _ = try std.fmt.hexToBytes(bytes[8 * N - nbytes ..], hex);
    var fe: Fe = @splat(0);
    for (0..N) |i| {
        fe[i] = std.mem.readInt(u64, bytes[8 * N - 8 * (i + 1) ..][0..8], .big);
    }
    return fe;
}

// -m^-1 mod 2^64 via Newton's iteration (doubles correct bits each step).
fn negInv64(m0: u64) u64 {
    var x: u64 = 1;
    inline for (0..6) |_| x = x *% (2 -% m0 *% x);
    return 0 -% x;
}

// R^2 mod m with R = 2^(64*N): start at 1 and double mod m 2*64*N times.
fn computeRR(m: *const Fe) Fe {
    var acc: Fe = @splat(0);
    acc[0] = 1;
    for (0..2 * 64 * N) |_| {
        var carry: u64 = 0;
        for (0..N) |i| {
            const nc = acc[i] >> 63;
            acc[i] = (acc[i] << 1) | carry;
            carry = nc;
        }
        if (carry != 0 or bi.geq(&acc, m)) {
            var borrow: u128 = 0;
            for (0..N) |i| {
                const d = @as(u128, acc[i]) -% @as(u128, m[i]) -% borrow;
                acc[i] = @truncate(d);
                borrow = (d >> 64) & 1;
            }
        }
    }
    return acc;
}

// The default key, used when no key is passed on the command line. The limbs are
// the big numbers from the ASN.1 PKCS#1 private key (the two primes, both CRT
// exponents and the coefficient); parsing still happens at runtime.
pub const default = struct {
    pub const p_hex = "";
    pub const q_hex = "";
    pub const dp_hex = "";
    pub const dq_hex = "";
    pub const qinv_hex = "";
};
